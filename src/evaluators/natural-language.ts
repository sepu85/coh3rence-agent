// src/evaluators/natural-language.ts
...
function extractProposalFromSignal(text: string): number | null {
  const match = text.match(/#p(\d+)/);
  return match ? parseInt(match[1], 10) : null;
}

const nlConsensusEvaluator: Evaluator = {
  name: "nl-consensus-evaluator",
  description:
    "Detects consensus signals, confusion, and records them with proposal context.",
    ...
    try {
      ...
      if (matchAny(text, CONFUSION_PATTERNS)) {
        ...
        if (targetProposal) {
          ...
          const helpAction = runtime.actions.find((a: any) => a.name === "help");
          if (helpAction) {
      ...
      let signal: "consent" | "stand-aside" | "block" | null = null;
      if (matchAny(text, CONSENT_PATTERNS)) signal = "consent";
      else if (matchAny(text, STAND_ASIDE_PATTERNS)) signal = "stand-aside";
      else if (matchAny(text, BLOCK_PATTERNS)) signal = "block";
...
async function checkForConsensusCompletion(
  runtime: any,
  proposal: any,
