...
async function saveMemory(opts: {
  ...
  userId: string;
  agentId: string;
  type: "proposal" | "concern" | "amendment" | "consensus-check" | "status";
  text: string;
}) {
...
/** ---------------- Concern ---------------- */
export const concernAction: Action = {
  name: "concern",
...
/** ---------------- Amendment ---------------- */
export const amendAction: Action = {
  name: "amend",
...
/** ---------------- Test Consensus ---------------- */
export const testConsensusAction: Action = {
  name: "test",
  description:
    "Move a proposal to testing stage and invite consensus responses.",
  ...
  examples: [
    [
      { user: "user", content: { text: "/test #p1" } },
      {
        ...
        content: {
          text:
            "🧭 #p1 moved to testing stage. Please respond with consent, concerns, or blocks.",
        },
      },
    ],
  ],
  ...
  handler: async (runtime: any, message: any) => {
    try {
      ...
      await saveMemory({
        ...
        text:
          `Consensus test initiated for ${proposalManager.formatProposalTag(proposalNumber)}. Please respond with consent, concerns, or blocks.`,
      });
    ...
    try {
      ...
      if (proposalNumbers.length > 0) {
        ...
        if (proposal.stage in STAGE_EXPLANATIONS) {
          statusText += STAGE_EXPLANATIONS[proposal.stage as keyof typeof STAGE_EXPLANATIONS](proposalNumber);
        }
